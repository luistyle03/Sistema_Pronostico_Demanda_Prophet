...
def probar_modulo_1(evaluador: EvaluadorDeModelos, exportador: ExportadorExcel) -> None:
    ...
    for r in evaluacion.resultados:
        if r.fallo:
            ...
            print(f"   {r.nombre_modelo:<18}{r.mape:>9.2f}{r.rmse:>9.2f}"
    ...
    filas = [{"Serie": serie.nombre, "Modelo": r.nombre_modelo,
              "MAPE (%)": None if r.fallo else round(r.mape, 2),
              "Ganador": "Sí" if r is ganador else "No", "Error": r.error or ""}
             for r in evaluacion.resultados]
    excel = exportador.exportar_evaluacion(filas, [])
    assert excel[:2] == b"PK", "Un .xlsx empieza con la firma ZIP 'PK'."
    print(f"   Excel de evidencia OK ({len(excel):,} bytes, "
          f"SHA-256 {sha256_de(excel)[:16]}…)")


def probar_modulo_2(generador: GeneradorDePronostico,
                    exportador: ExportadorExcel) -> None:
    ...
    assert pronostico.fechas[0] == serie.puntos[-1].fecha + timedelta(days=1), \
        "El pronóstico empieza el día siguiente al último histórico."
    print(f"   Total proyectado: {resumen.total_proyectado:,.1f} uds. | "
...
if __name__ == "__main__":
    modelos = [AdaptadorProphet(ParametrosPronostico()), AdaptadorARIMA(),
               AdaptadorHoltWinters(), AdaptadorMediaMovil(),
               AdaptadorRegresionLineal()]
    probar_modulo_1(EvaluadorDeModelos(modelos, AdaptadorPruebasScipy()),
                    ExportadorExcel())
